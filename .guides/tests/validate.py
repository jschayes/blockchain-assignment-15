...
def sign_and_send(contract, function, signer, argdict, confirm=True, force_nonce=0):
    ...
    if confirm:
        ...
        if tx_receipt.status:
            print(
                f"{bcolors.OKGREEN}SUCCESS{bcolors.ENDC}: in sign_and_send, Transaction confirmed for '{function}' at block {tx_receipt.blockNumber}")
        else:
...
def make_deposits(deposits, source_contract):
    ...
    for d in deposits:
        ...
        try:
            transaction_hash, _ = sign_and_send(token,
                                                "approve",
                                                sender,
            ...
            print(
                f"{bcolors.FAIL}ERROR{bcolors.ENDC}: Failed to approve token transfer\nContact your instructor\n{e}\n")

        try:
            transaction_hash, _ = sign_and_send(source_contract,
                                                "deposit",
                                                sender,
...
def make_withdrawals(withdrawals, destination_contract):
    ...
    for d in withdrawals:
        ...
        try:
            transaction_hash, _ = sign_and_send(destination_contract,
                                                "unwrap",
                                                sender,
                                                {'_wrapped_token': token.address, '_recipient': receiver,
                                                 '_amount': amount},
                                                force_nonce=nonce)
            nonce += 1
